...
class TestCommandLine:
    ...
    def test_execution(self, tmp_path):
        ...
        (errno, stdout, stderr) = execute_sheet2linkml([
            '--google-sheet-id', '1HpsF12vl_CeIzV2TEtANeysnh7cy6YOUDdv6fMXT00I',
            '-o', output_path
        ])

        assert 'INFO: Google Sheet loaded: GSheetModel with an underlying Google Sheet titled "Test schema for sheet2linkml" containing 8 worksheets' in stderr
        assert 'INFO: Generating LinkML for GSheetModel with an underlying Google Sheet titled "Test schema for sheet2linkml" containing 8 worksheets' in stderr
        assert 'INFO: Generating LinkML for Enum named Subject.breed from worksheet "O_CCDH Enums" containing 8 values' in stderr
        ...
        with open(os.path.join(dirname, 'testing-schema-on-gsheet/1HpsF12vl_CeIzV2TEtANeysnh7cy6YOUDdv6fMXT00I.yaml'), 'r') as f:
